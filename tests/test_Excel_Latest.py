...
class TestExcelCase:
    ak = ApiKey()
    AllCaseData = FileReader.read_excel_to_dict(sheet_name="Sheet1")
    ...
    def __dynamic_title(case_data):
        ...
        if case_data["CaseName"] is not None:
            allure.dynamic.title(f"ID({case_data['ID']}) - {case_data['CaseName']}")
    ...
    def test_case(self, case_data):
        ...
        try:
            ...
            params = eval(Template(case_data["Params"]).substitute(self.all_extract)) if isinstance(case_data["Params"],
                                                                                                    str) else None
            headers = eval(Template(case_data["Headers"]).substitute(self.all_extract)) if isinstance(
                case_data["Headers"], str) else None
            # 处理加密字段数据, 在需要加密的字段前加入@标识, 若没有@标识，则返回原数据
            data = FileReader.data_encrypt_by_aes(
                eval(Template(case_data["Data"]).substitute(self.all_extract))) if isinstance(case_data["Data"],
                                                                                              str) else None
